...
|-------------------------------------|---------------------|
| **1** — Фундамент: backend и модель данных | ✅ Закрыт по backend: стек, контракт iter-02, каркас, **полная реализация API этапа 1** (PostgreSQL), репозитории, тесты, документация; см. статус этапа 1 выше |
...
| [iter-04](#iter-04--api-tests-mvp-тесты-api) | Тесты API | ✅ done | iter-03 | `tests/`: MVP (слоты, запись) + интеграция **визит/бонусы** на PostgreSQL |
| [iter-05](#iter-05--impl-endpoints-реализация-endpoints) | Реализация endpoints | ✅ done | iter-04 | MVP → **полный срез этапа 1** (PG): роутеры, сервисы, репозитории, ORM |
...
**Ценность:** Бот, веб и админка могут проектироваться против единого контракта; изменения — осознанные. Реализация endpoint’ов шла поэтапно (**iter-05**: MVP → полный срез этапа 1 для PostgreSQL).
...
**Цель:** Реализовать серверные endpoint’ы по **контракту iter-02**. **Исходная волна (MVP):** слоты, запись, каталог услуг, in-memory/SQLite. **Завершение этапа 1:** при `DATABASE_URL` PostgreSQL — **все маршруты OpenAPI** для клиента и администратора (см. матрицу iter-02), репозитории SQLAlchemy, подтверждение визита и лояльность, разнесение роутов по модулям.
...
- [x] Авторизация: Bearer `mvp-<uuid>` после auth; `BOT_SECRET` + `X-Telegram-User-Id`; админ — `ADMIN_API_TOKEN` + [`deps_extra`](../../backend/src/pereobuyka/api/v1/deps_extra.py) (`SessionPg`, `AdminActor`)
...
- Тесты iter-04 зелёные; логика слотов/записи/визитов/бонусов в `services/` и `storage/repositories/`, не в handlers бота
...
- Бот: перевести сценарии с `BOT_SECRET` на пользовательский токен `mvp-*` там, где нужен профиль в БД; вызовы новых endpoint’ов этапа 1
...
| iter-04 (целиком) | Тесты API: health, слоты, запись; PG + сценарий визита/бонусов | ✅ | [`impl/backend/iteration-04-api-tests-mvp/`](impl/backend/iteration-04-api-tests-mvp/) |
| iter-05 (целиком) | Реализация endpoints: MVP → **полный API этапа 1 (PG)**, репозитории, endpoints-пакет | ✅ | [`impl/backend/iteration-05-impl-endpoints/`](impl/backend/iteration-05-impl-endpoints/) |
