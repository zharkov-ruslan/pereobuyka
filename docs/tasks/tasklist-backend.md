...
| **2** — Telegram-бот | Пересечение: **iter-07** — бот переводится на HTTP к backend; остальное — отдельные tasklist’ы по боту/интеграциям |
...
| [iter-04](#iter-04--api-tests-базовые-тесты-api) | Тесты API | ✅ done | iter-03 | `tests/`: базовый срез (слоты, запись) + интеграция **визит/бонусы** на PostgreSQL |
| [iter-05](#iter-05--impl-endpoints-реализация-endpoints) | Реализация endpoints | ✅ done | iter-04 | базовый срез → **полный срез этапа 1** (PG): роутеры, сервисы, репозитории, ORM |
...
**Ценность:** Бот, веб и админка могут проектироваться против единого контракта; изменения — осознанные. Реализация endpoint’ов шла поэтапно (**iter-05**: базовый срез → полный срез этапа 1 для PostgreSQL).
...
### iter-04 — API tests (базовые тесты API)
...
**Цель:** Рефакторинг Telegram-бота: тонкие handlers, вызовы backend HTTP API для сценариев слотов и записи (базовый срез), а по мере появления реализации — для остальных сценариев, покрытых API iter-05.
...
| iter-05 (целиком) | Реализация endpoints: базовый срез → **полный API этапа 1 (PG)**, репозитории, endpoints-пакет | ✅ | [`impl/backend/iteration-05-impl-endpoints/`](impl/backend/iteration-05-impl-endpoints/) |
