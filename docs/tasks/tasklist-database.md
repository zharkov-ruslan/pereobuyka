...
- In-memory не остаётся единственным хранилищем для целевых endpoint’ов без явного ADR/заметки о временном исключении
