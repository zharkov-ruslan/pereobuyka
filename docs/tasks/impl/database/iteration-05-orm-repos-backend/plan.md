...
1. Пакет `pereobuyka.db`: `Base`, ORM-модели под таблицы базового среза (`users`, `services`, `schedule_rules`, `appointments`, `appointment_services`), async `create_engine` / `async_sessionmaker`, зависимость `get_db_session` (commit/rollback на запрос).
