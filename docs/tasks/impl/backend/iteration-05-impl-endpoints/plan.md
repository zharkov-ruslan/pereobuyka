# iter-be-05 — Impl endpoints: план

## Контекст

В iter-be-04 реализован базовый код эндпоинтов (слоты, запись) в виде "stub-реализации" с пометками `# iter-be-05`. Вся бизнес-логика уже находится в `services/`, обработка ошибок подключена, тесты зелёные. iter-be-05 верифицирует и формализует это как законченный базовый слой, исправляет оставшиеся замечания и расширяет каталогом услуг.

## Что будет сделано

### 1. Формализация временного слоя хранилища

`tasklist-database.md` не существует → БД-слой не планируется в текущем цикле. Временный in-memory storage фиксируется как **явная архитектурная позиция** (не "заглушка для iter-be-05", а "временный слой до появления database-tasklist"). Комментарии в коде обновляются соответственно.
...
Добавляется endpoint из сценария **Клиент 3** (`iter-be-02`): список активных услуг для UI и расчёта.
...
Исходный план выше описывает **первую волну** (базовый срез + `GET /services`). По [`tasklist-backend.md`](../../../tasklist-backend.md) итерация iter-be-05 дополнена **полным срезом API для PostgreSQL**:
...
- интеграционные тесты вместе с iter-be-04: **16 passed** при прогоне с PostgreSQL (Testcontainers).
