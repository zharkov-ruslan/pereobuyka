...
- **`test_services.py`** — 3 теста на каталог услуг (добавлено в рамках iter-be-05, остаётся в общем прогоне)
...
### Stub-реализация базовых endpoint'ов (iter-be-04)
...
Исходный `plan.md` предполагал только in-memory и `reset_storage`. Фактически для стабильной проверки этапа 1 тесты опираются на **Testcontainers PostgreSQL** (см. `conftest.py`). Это зафиксировано в [`tasklist-backend.md`](../../../tasklist-backend.md) (iter-be-04).
