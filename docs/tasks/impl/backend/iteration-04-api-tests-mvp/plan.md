# iter-04 — API tests: план

## Цель

Написать автотесты на базовые endpoint'ы (`/health`, `GET /api/v1/slots`, `POST /api/v1/appointments`) и обеспечить их зелёный прогон. Логика реализована в in-memory хранилище (stub), не в БД — это явная временная точка, которую iter-05 заменит на реальный storage.
