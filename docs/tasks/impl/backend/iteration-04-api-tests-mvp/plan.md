# iter-04 — API tests MVP: план

## Цель

Написать автотесты на MVP-endpoint'ы (`/health`, `GET /api/v1/slots`, `POST /api/v1/appointments`) и обеспечить их зелёный прогон. Логика реализована в in-memory хранилище (stub), не в БД — это явная временная точка, которую iter-05 заменит на реальный storage.
...
| Файл | Назначение |
|------|-----------|
| `backend/src/pereobuyka/api/v1/schemas.py` | Pydantic-схемы по контракту iter-02 (SlotWindow, Appointment, …) |
...
Эти допущения явно помечены в коде (`# iter-05: заменить на реальный storage`).
...
После расширения backend ([`tasklist-backend.md`](../../../tasklist-backend.md), iter-04–iter-05) прогон тестов включает **PostgreSQL** (Testcontainers) в [`backend/tests/conftest.py`](../../../../../backend/tests/conftest.py), файл `test_visit_confirm.py` и **16** успешных тестов на сессию. Исходная цель итерации (MVP: health, слоты, запись на stub) сохраняется; интеграционные проверки визита/бонусов добавлены как следующий слой покрытия.
