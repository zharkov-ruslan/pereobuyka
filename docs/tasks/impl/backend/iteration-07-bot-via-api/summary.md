# iter-be-07 — Bot via API: итоги реализации
...
По сравнению с первоначальным планом iter-be-07: исходники бота лежат в `bot/src/pereobuyka/`, а не в корневом `src/` — отдельная задача по выравниванию структуры репозитория. Остальное соответствует плану.
...
- **`MemoryStorage` для FSM** — достаточно для текущего среза; при рестарте бота пользователи начинают сессию заново (ожидаемо).
