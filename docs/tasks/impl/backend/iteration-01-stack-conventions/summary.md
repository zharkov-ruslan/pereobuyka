# Summary: iter-01 — Stack & conventions
...
- **`tasklist-backend.md`**: iter-01 отмечен выполненным; артефакты включают ADR-002 и ADR-003.
...
- iter-03: каркас `backend/` с `fastapi`, ASGI-сервером (например `uvicorn`), зависимости ORM/миграций по ADR-003 в `pyproject.toml`.
